...
该脚本会优先使用当前可用的已配置或推荐文本模型，真实调用一次七牛 AI，并检查模型来源、场次数、质量门禁和作者复核警告；只输出非敏感摘要，不输出密钥或 AI 生成正文。需要指定模型时可追加 `-Model deepseek-v3`。若模型不遵守完整改编输出契约，脚本会如实失败，不会以兜底结果冒充 AI 成功。
